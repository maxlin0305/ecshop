...
class InvoinceJob extends Job
{
    ...
    public function handle(): void
    {
        ...
        $req = [
            ...
            'CustomerName' => $result['orderInfo']['invoice']['customer_name']??null,
            'CustomerAddr' => $result['orderInfo']['invoice']['customer_address']??null,
        ...
        app('log')->debug('ecpay invoice responsedataA:' . to_json($req));
        ...
        app('log')->debug('ecpay invoice hash_key:' . config('ecpay.invoice.hash_key'));
        app('log')->debug('ecpay invoice hash_iv:' . config('ecpay.invoice.hash_iv'));
        app('log')->debug('ecpay invoice url:' . $uri);
        $arParameters = [
            'MerchantID' => config('ecpay.merchant_id'),
            'RqHeader' => [
                'Timestamp' => time(),
        ...
        app('log')->debug('ecpay invoice requestparams----:' . to_json($result));
