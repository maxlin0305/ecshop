...
use GoodsBundle\Api\V1\Action\ItemsCategory;
...
use PopularizeBundle\Services\SettingMerchantService;
...
use GoodsBundle\Entities\Items as ItemsModel;
...
class Items extends BaseController
{
    ...
    public function getShopItemsList(Request $request)
    {
        ...
        if ($categoryId) {
            ...
            if (!$itemsRelCatsList['total_count']) {
                return $this->response->array(['total_count' => 0, 'list' => []]);
            }
            $params['item_id'] = [];
        ...
        if ($config['goods'] == 'select') {
            $whereOrSql[] = $qb->expr()->andX(
                $qb->expr()->eq('distributor_id', 0),
                $qb->expr()->eq('rebate', 1),
            );
        }else{
            $whereOrSql[] = $qb->expr()->andX(
        ...
        $settingMerchantService = new SettingMerchantService();
        $allConfig = $settingMerchantService->getAllOpenedDistribution();
        foreach (array_column($allConfig, 'goods', 'distributorId') as $k => $v){
            if ($v == 'select') {
                $whereOrSql[] = $qb->expr()->andX(
                    $qb->expr()->eq('distributor_id', $k),
                    $qb->expr()->eq('rebate', 1),
                );
            }else{
                $whereOrSql[] = $qb->expr()->andX(
        ...
        if (count($lists) === 0) {
            ...
            if (count($params['item_id']??[]) !== 0){
                $params['item_id'] = array_intersect($params['item_id'], $allItemIds??[]);
            }else{
                $params['item_id'] = $allItemIds??[];
            }
            if (count($params['item_id']) == 0) {
                $params['item_id'] = [-1];
            }
...
//            $params['rebate'] = 1;
        ...
        if (isset($inputData['is_promoter']) && $inputData['is_promoter'] && $result['list']) {
            ...
            foreach ($result['list'] as &$itemInfo) {
                if ($itemInfo['distributor_id']  == 0){

                    $itemInfo = $this->__replaceItempPomoterPrice($itemInfo, $config);
                }else{
                    $newConfig = $settingMerchantService->getConfig($itemInfo['distributor_id']);
                    $itemInfo = $this->__replaceItempPomoterPrice($itemInfo, $newConfig);
                }
