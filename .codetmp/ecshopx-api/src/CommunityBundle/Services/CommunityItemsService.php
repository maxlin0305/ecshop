    ...
    public function getItemsList($filter, $page = 1, $pageSize = 2000, $orderBy = ['item_id' => 'DESC'])
    {
        ...
        foreach ($itemsList['list'] as $key => &$v) {
            ...
            if ($v['distributor_id'] == 0){
                $v['distributor_name'] = "平臺";
            }else{
                $distributorInfo = $this->distributor->getInfo(['distributor_id'=>$v['distributor_id']]);
                $v['distributorInfo'] = $distributorInfo;
                $v['distributor_name'] = $distributorInfo['name']??'';
            }
