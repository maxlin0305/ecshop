...
class NormalOrderCancelAppMsgNotify extends BaseListeners implements ShouldQueue
{
    ...
    {
        app('log')->info('[app消息]触发订单取消通知:'.json_encode($event,JSON_UNESCAPED_UNICODE));
        $companyId = $event->entities['company_id'];
        $orderId = $event->entities['order_id'];
        app('log')->info('[app消息]触发订单取消通知，订单ID:'.$orderId);
        $this->doSendAppMsg($orderId,$companyId);
        app('log')->info('[app消息]触发订单取消通知完成，订单ID:'.$orderId);
    }
     ...
     * @throws \Exception
     */
    private function doSendAppMsg($orderId,$companyId)
    {
        ...
        if($status == 1 ){
            ...
            $request_params = [
                ...
                'data' => [
                    ...
                    "orderCancelDate"=>date("Y-m-d H:i:s",time())
                ],
            ];
            ...
            $typeName = '訂單取消通知:'.$orderId;
            $pushMessageService->createPushMessage('post',
                $request_url,$request_params,3,$typeName,
