    ...
    public function taskBrokerageList($list)
    {
        ...
        if ($taskBrokerageList['list']) {
            $orderStatus = [
                ...
                'REVIEW_PASS' => '審核通過待出庫',
                'WAIT_GROUPS_SUCCESS' => '等待成團',
                'REFUND_PROCESS' => '退款處理中',
            ];
            $statusMsg = [
                'wait' => '待統計',
                'finish' => '已統計',
        ...
        for ($page = 1; $page <= $fileNum; $page++) {
            ...
            foreach ($list['list'] as $key => $value) {
                ...
                foreach ($title as $k => $v) {
                    if ($k == 'status') {
                        $data[$key][$k] = $value[$k] ? '已達標' : $value['limit_desc'];
                    } elseif ($k == 'rebate_money' || $k == 'total_fee') {
                        $data[$key][$k] = 'NT$'.$value[$k] / 100;
                    } elseif ($k == 'rebate_type') {
                        $data[$key][$k] = $value[$k] == 'total_num' ? '按總數量' : '按總金額';
