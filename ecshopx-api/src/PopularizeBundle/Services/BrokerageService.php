...
class BrokerageService
{
    ...
    protected function promoterLevelBrokerage()
    {
        ...
        $companyId = $this->orderInfo['orderInfo']['company_id'];
        $userId = $this->orderInfo['orderInfo']['user_id'];
        app('log')->debug('------------------：'. json_encode($this->orderInfo));

        $distributor_id = $this->orderInfo['orderInfo']['distributor_id'];
        $settingService = new SettingService();
        $settingMerchantService = new SettingMerchantService();
        $isOpen = $settingMerchantService->getOpenPopularize($distributor_id);
        ...
        if ($distributor_id != 0) {
            $merchantConfig = $settingMerchantService->getConfig($distributor_id);
...
//        $this->plan_close_time = time() + 3600 * 24 * ($settleTime + $latestAftersaleTime);
    ...
    private function getItemsRebateConf()
    {
        ...
        if (isset($orderInfo['items'])) {
            ...
            foreach ($itemList['list'] as $row) {
                ...
                if (($this->rebateGoodsMode == 'all') || ($defaultItemsRebate[$row['default_item_id']] ?? 0)) {
    ...
    public function scheduleSettleRebate()
    {
        ...
        app('log')->debug('定时执行佣金结算开始=>totalCount'.$totalCount);
        if (!$totalCount) {
            return true;
        }
        ...
        for ($i = 1; $i <= $totalPage; $i++) {
            ...
            app('log')->debug('定时执行佣金结算开始=>'.var_export($data['list'],1));
            foreach ($data['list'] as $row) {
                app('log')->debug('定时执行佣金结算完成--');
//                $conn = app('registry')->getConnection('default');
//                $conn->beginTransaction();
                app('log')->debug('定时执行佣金结算完成--beginTransaction');
                try {
                    app('log')->debug('定时执行佣金结算完成--updateOneBy---');
                    $this->brokerageRepository->updateOneBy(['id' => (int)$row['id']], ['is_close' => true]);
                    app('log')->debug('定时执行佣金结算完成--updateOneBy');
                    // 根据类型结算
                    if ($row['commission_type'] == 'money') {
                        ...
                        app('log')->debug('定时执行佣金结算完成--updateSettlePoint');
