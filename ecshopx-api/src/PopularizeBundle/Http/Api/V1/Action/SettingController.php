...
use OrdersBundle\Services\ConsumptionOrderService;
...
class SettingController extends Controller
{
    ...
    public function setMerchantConfig(Request $request) {

        $distributorId = app('auth')->user()->get('distributor_id');

        $isOpenPopularize = $request->input('isOpenPopularize', false);
        $settingService = new SettingMerchantService();
        $settingService->openPopularize($distributorId, $isOpenPopularize);
        $data = $request->input();
        $settingService->setConfig($distributorId, $data);
    ...
    public function getMerchantConfig(Request $request) {
        ...
        $distributorId = app('auth')->user()->get('distributor_id');
        $settingService = new SettingMerchantService();
        $config = $settingService->getConfig($distributorId);
        $config['isOpenPopularize'] = $settingService->getOpenPopularize($distributorId);
...
//
//
//        app('log')->debug('定时执行佣金结算开始=>BrokerageService');
//        $brokerageService = new BrokerageService();
//        $brokerageService->scheduleSettleRebate();
//        app('log')->debug('定时执行佣金结算开始=>BrokerageService_env');
//
//        echo 1111;
    ...
    public function bbb(Request $request) {
        ...
        $pointMemberService = new PointMemberService();
        $pointMemberService->expirationPoints();
        echo 1111;

    }
    public function ccc(Request $request) {

        $pointMemberService = new ConsumptionOrderService();
        $pointMemberService->handleMembershipLevel();
