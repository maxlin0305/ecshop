...
//            "habbit.*.ischecked" => ["required", "boolean"],
    ...
    */
    public function batchProcess(Request $request)
    {
        ...
        if (!$userId){
            throw new ErrorException(ErrorCode::MEMBER_ERROR, "会员ID必填");
        }
...
//        $filter['distributor_id'] = 1;
        ...
        $gradeForm = json_decode($request->post('vip_grade_form'), true);
        $params['vip_grade_id'] = $gradeForm['vip_grade_id'];
        $params['add_day'] = intval($gradeForm['add_day']);
        // 判断day是否为正整数
        if ($params['add_day'] <= 0) {
            throw new ErrorException('请填写正确的延期天数');
        }
        $vipGradeService = app('registry')->getManager('default')->getRepository(VipGrade::class);;
        $info = $vipGradeService->getInfo(['vip_grade_id' => $params['vip_grade_id'], 'company_id' => 1]);
        if (!$info) {
...
//            return $this->expiredMemberExtension($companyId, $info, $params);
        ...
        $memberService = new \MembersBundle\Services\MemberService();
        ...
        if ($count > 100) {
            ...
            foreach ($userIds as $ids) {
                $filter['user_id'] = $ids;
                $queue = (new BatchActionMembers($params, 'vip_delay', $filter, true))->onQueue('slow');
                app('Illuminate\Contracts\Bus\Dispatcher')->dispatch($queue);
            }
            return $this->response->array(['status' => true, 'msg' => '由于数据较多，已经加入队列处理']);
        } else {
            $queue = new BatchActionMembers($params, 'vip_delay', $filter, false);
            $queue->handle();
