...
class MemberService
{
     ...
     * 更新微信会员总消费额New
     */
    public function updateMemberConsumptionNew($userId, $pay_fee)
    {
...
//        $totalConsumption = $this->getTotalConsumption($userId);
        ...
        $companyId = 1;
        $memberCardService = new MemberCardService();
        $gradeList = $memberCardService->getGradeListByCompanyId(1);
        //按照消费额度从大到小
        krsort($gradeList);
        foreach ($gradeList as $key => $value) {
            $condition = $value['promotion_condition']['total_consumption'] ?? 0;
    ...
    public function updateMemberMobile($params, $filter)
    {
        ...
        $sql = "update members set mobile = '{$params['mobile']}',region_mobile= '{$params['mobile']}' where user_id = {$filter["user_id"]};";
        $conn = app('registry')->getConnection('default');
    ...
    */
    ...
    public function getTokenData(array $userInfo): array
    {
        return [
            ...
            "app_member_id" => $userInfo["app_member_id"],
