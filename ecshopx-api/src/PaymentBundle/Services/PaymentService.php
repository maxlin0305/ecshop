...
class PaymentService
{
    ...
    public function payment($authInfo, $params)
    {
        ...
        if(empty($orderInfo)) {
            throw new BadRequestHttpException('當前訂單不存在');
        }
        if (!in_array($orderInfo['order_status'], ['NOTPAY', 'PART_PAYMENT'])) {
           // throw new BadRequestHttpException('當前訂單不需要支付');
        }
        ...
        if ('deposit' == $params['pay_type']) {
            if (!isset($authInfo['user_card_code']) || !$authInfo['user_card_code']) {
                throw new BadRequestHttpException('請先登錄');
            }
        ...
        if ($params['pay_type'] == 'alipaymini') {
            if (!isset($authInfo['alipay_user_id']) || !$authInfo['alipay_user_id']) {
                throw new BadRequestHttpException('請在支付寶小程序授權登錄');
            }
    ...
    public function getToken($authInfo, $params)
    {
        $params['pay_type'] = ($params['pay_type'] ?? '') ? $params['pay_type'] : 'ecpay_h5';

        if($params['pay_type'] != 'ecpay_h5'){
            throw new BadRequestHttpException('僅綠界支持獲取token');
        }
        ...
        $authorizerAppId = $authInfo['woa_appid'] ?? '';
        $wxaAppId = $authInfo['wxapp_appid'] ?? '';
        $service = $this->getPaymentService($params['pay_type']);
        return $service->getToken($authorizerAppId, $wxaAppId, $data, false);
    }

    // 根据前端的payToken发起交易
    public function paymentByPayToken($params)
    {
        $params['pay_type'] = ($params['pay_type'] ?? '') ? $params['pay_type'] : 'ecpay_h5';
        $service = $this->getPaymentService($params['pay_type']);
        return $service->paymentByPayToken($params);
    }
