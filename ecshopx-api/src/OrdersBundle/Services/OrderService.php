...
//            $orderData['invoice']['type'] = $params['invoice_type'];
              $orderData['invoice'] = $this->__getInvoiceData($params['invoice_type'], $params['invoice_content']);
        }
     ...
     * @throws \Exception
     */
    private function __getInvoiceData($invoiceType, $invoiceContent)
    {
        $data = [
            'type'=>$invoiceType
        ];
        if($invoiceContent['title'] == 'individual'){
            $data['carrier_type'] = $invoiceContent['carrier_type'];//发票类型
            //个人发票
            switch ($invoiceContent['carrier_type']){
                case 1://电子发票
                    $data['customer_name'] = $invoiceContent['customer_name'];
                    $data['customer_address'] = $invoiceContent['customer_address'];
                    $data['customer_phone'] = $invoiceContent['customer_phone'];
                    $data['customer_email'] = $invoiceContent['customer_email'];
                    break;
                case 2://自然人
                    $data['carrier_num'] = $invoiceContent['carrier_num'];
                    break;
                case 3://手机条码
                    $data['carrier_num'] = $invoiceContent['carrier_num'];
                    break;
                default:
                    throw new \Exception('未知的个人发票类型');
            }
        }else if($invoiceContent['title']=='unit'){
            //公司发票
            $data['customer_name'] = $invoiceContent['customer_name'];
            $data['customer_address'] = $invoiceContent['customer_address'];
            $data['customer_phone'] = $invoiceContent['customer_phone'];
            $data['customer_email'] = $invoiceContent['customer_email'];
            $data['carrier_num'] = $invoiceContent['carrier_num'];
            $data['customer_identifier'] = $invoiceContent['customer_identifier'];
        }else{
            throw new \Exception('未知的发票类型');
        }
        return $data;
    }
    /**
