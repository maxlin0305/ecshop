...
class MarketingGoodsUploadService
{
    ...
    public $headerInfo = [
        ...
        '庫存' => [
            'size' => 255,
            'remarks' => '庫存為大於0的整數，請注意是整數，小數或浮點數將向上取整',
            'is_need' => false,
        ],
        '每人限購' => [
            'size' => 255,
    ...
    private function getItemParams($companyId, $row)
    {
        ...
        if ($row['item_params']) {
            ...
            if ($attrValuesList['total_count'] > 0) {
                ...
                throw new BadRequestHttpException('商品參數值不存在');
            }
        }
    ...
    private function getItemSpec($companyId, $row)
    {
        ...
        if ($row['item_spec']) {
            ...
            foreach ($itemParams as $row) {
                $itemRow = explode(':', $row);
                if (!$itemRow[0]) {
                    throw new BadRequestHttpException('存在無效的商品規格');
                }
                if (!$itemRow[1]) {
                    throw new BadRequestHttpException('存在無效的商品規格值');
                }
                $attributeNames[] = $itemRow[0];
                $attributeValues[] = $itemRow[1];
            }
